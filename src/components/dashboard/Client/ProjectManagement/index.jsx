...
export default function ClientProjectManagement() {
  ...
  // Fetch projects and vendors data
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (!session?.user?.email) {
          setError("Please log in as a client to view projects");
          setLoading(false);
          return;
        }
        ...
        const projectsResponse = await fetch(
          `/api/client/projectManagement?clientEmail=${session?.user?.email}&status=${filterStatus}`
        );
  ...
  const handleAddProject = async () => {
    ...
    try {
      const response = await fetch("/api/client/projectManagement", {
        method: "POST",
        headers: {
  ...
  const handleDeleteProject = async (id) => {
    try {
      const response = await fetch(
        `/api/client/projectManagement?id=${id}&clientEmail=${session?.user?.email}`,
        {
          method: "DELETE",
        }
      );
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600 mt-2">Please make sure you are logged in as a client.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Manage your projects with vendors
            </p>
          </div>
          <Button
