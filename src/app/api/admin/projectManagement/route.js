...
export async function POST(req) {
  try {
    const { title, description, vendorEmail, clientEmail, budget, deadline } = await req.json();

    if (!title || !description || !vendorEmail || !clientEmail || !budget) {
      return NextResponse.json(
        { error: "Title, description, vendor email, client email, and budget are required" },
        { status: 400 }
      );
    }
